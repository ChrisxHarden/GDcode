...
class QMixer(nn.Module):
    def __init__(self, args):
        ...
        state_shape=[self.args.obs_shape[0],self.n_agents]
        self.state_dim = int(np.prod(state_shape))

        self.embed_dim = args.mixing_embed_dim
        self.q_embed_dim = getattr(self.args, "q_embed_dim", 1)

        self.hyper_w_1 = nn.Linear(self.state_dim, self.embed_dim * self.n_agents * self.q_embed_dim)#一层的默认设置
    ...
    def forward(self, agent_qs, states):
        ...
        # First layer
        w1 = th.abs(self.hyper_w_1(states))#batch_size,emd_dim*n_agents
        b1 = self.hyper_b_1(states)#batch_size,emd_dim
        w1 = w1.view(-1, self.n_agents*self.q_embed_dim, self.embed_dim)#batch_size,n_agents,emd_dim
        b1 = b1.view(-1, 1, self.embed_dim)#batch_size,emd_dim
        hidden = F.elu(th.bmm(agent_qs, w1) + b1)#batch_size,1,emd_dim
        # Second layer
        w_final = th.abs(self.hyper_w_final(states))#batch_size,emd_dim
        w_final = w_final.view(-1, self.embed_dim, 1)#batch_size,emd_dim,1
        # State-dependent bias
        v = self.V(states).view(-1, 1, 1)##batch_size,1,1
        # Skip connections
        s = 0
        if self.args.skip_connections:
            s = agent_qs.sum(dim=2, keepdim=True)#batch_size,1,1

        if self.args.gated:
            y = th.bmm(hidden, w_final) * self.gate + v + s#batch_size,1,1
        else:
