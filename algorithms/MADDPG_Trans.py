import torch
import os
from modules.agents.transformer_agent import Actor as transformerActor
...
class MADDPGTrans:
    def __init__(self, args, agent_id):  # 因为不同的agent的obs、act维度可能不一样，所以神经网络不同,需要agent_id来区分
        ...
        # create the network
        self.actor_network = transformerActor(args, agent_id)
        self.critic_network = Critic(args)

        # build up the target network
        self.actor_target_network = transformerActor(args, agent_id)
