...
class FACMAClearner:
    ...
    def train(self, transitions, other_agents):
                    ...
                    if agent_id == self.agent_id:
                        u_next.append(
                            self.actor_target_network.forward(o_next[agent_id], self.actor_target_network.hidden_state)[
                                "actions"])
                        q_next_agent_a = self.critic_target_network.forward(o_next, u_next[agent_id]).detach()
                        q_next.append(q_next_agent_a)
                    else:
        ...
        if not self.args.share_param:
            for agent_id in range(self.args.n_agents):
                if agent_id == self.agent_id:
                    ...
                    q_agent_a = self.critic_network.forward(o, u[agent_id])
                    q.append(q_agent_a)
                else:
        ...
        critic_loss_tot = (target_q - q).pow(2).mean()
        # critic_loss_agent = (q_next_agent_a - q_agent_a).pow(2).mean()
        # critic_loss=critic_loss_tot+critic_loss_agent

        self.critic_optim.zero_grad()
        critic_loss_tot.backward()
        ...
        if self.train_step > 0 and self.train_step % self.args.save_rate == 0:
            ...
            print('critic_loss is {}, actor_loss is {}'.format(critic_loss_tot, actor_loss))
        self.train_step += 1
