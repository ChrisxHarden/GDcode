...
class FACMACSCH:
    def __init__(self, args, agent_id):  # 因为不同的agent的obs、act维度可能不一样，所以神经网络不同,需要agent_id来区分
        ...
        self.actor_optim_tot = torch.optim.Adam(self.actor_network.parameters(), lr=self.args.lr_actor*0.1)
        self.critic_optim = torch.optim.Adam(self.critic_network.parameters(), lr=self.args.lr_critic)
        self.critic_optim_tot = torch.optim.Adam(self.critic_param, lr=self.args.lr_critic*0.1)
    ...
    def train(self, transitions, other_agents):
        ...
        with torch.no_grad():
            ...
            if not self.args.share_param:
                ...
                for agent_id in range(self.args.n_agents):
                    ...
                    q_next_agent = self.critic_target_network.forward(o_next, u_next[agent_id]).detach()
                    q_next.append(q_next_agent)
                q_next_agent_a=q_next[self.agent_id]




            q_next = torch.stack(q_next, dim=-1)  # (batch_size,1,n_agent)
            # q_next.squeeze()
            ...
            q_next_agent_a=(r.unsqueeze(1) + self.args.gamma * q_next_agent_a).detach()
        ...
        if self.train_step<self.args.time_steps/2:
            critic_loss=(q_next_agent_a - q_agent_a).pow(2).mean()
            ...
            self.critic_optim_tot.zero_grad()
            critic_loss.backward()
            self.critic_optim_tot.step()
        ...
        if not self.args.share_param:
            for agent_id in range(self.args.n_agents):
                if agent_id == self.agent_id:
                    ...
                    q_agent_val_a = self.critic_network.forward(o, actions_taken[agent_id])
                    q_val_of_actions_taken.append(q_agent_val_a)
                else:
            ...
            for agent_id in range(self.args.n_agents):
                ...
                q_val_of_actions_taken.append(q_agent)
            q_agent_val_a=q_val_of_actions_taken[self.agent_id]
        ...
        if self.train_step<self.args.time_steps:
            actor_loss=-q_agent_val_a.mean()
            self.actor_optim.zero_grad()
            actor_loss.backward()
            self.actor_optim.step()
        else:
            actor_loss = -q_val_of_actions_taken.mean()
            self.actor_optim_tot.zero_grad()
            actor_loss.backward()
            self.actor_optim_tot.step()
